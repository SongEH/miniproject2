...
<script type="text/javascript">
    ...
    alert(f.a_idx.value);
    alert(f.a_content.value);
    alert(f.b_idx.value);
    alert(f.b_cate.value);
...
<%@ include file="/WEB-INF/views/top.jsp" %>
    <form  accept-charset="utf-8">
        <c:forEach var="vo" items="${list}">
            <div class="container" id="answer_${vo.a_idx}">
            <input type="hidden" name="a_idx" value="${vo.a_idx }">
            <input type="hidden" name="a_content" value="${vo.a_content }">
            <input type="hidden" name="b_idx" value="${vo.b_idx }">
            <input type="hidden" name="b_cate" value="${vo.b_cate }">
                <article>
                    <div class="col-md-10 col-lg-10" style="text-align: right; overflow: auto;">
                        <div>
                            ${vo.m_name} (
                            <fmt:parseDate value="${vo.a_rdate}" pattern="yyyy-MM-dd HH:mm:ss." var="parsedDate" />
                            <fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${parsedDate}" />
                            )
                            <div class="post-description">
                            ${vo.a_idx } ${vo.a_content}
                            </div>
                    ...
                    <div class="col-md-2 col-lg-2">
                        ...
                        <c:if test="${user.m_idx eq vo.m_idx}">
                            <div>
                                <button type="button" class="btn btn-danger" onclick="answer_delete(${vo.a_idx});">삭제</button>
                                <button type="button" class="btn btn-warning" onclick="answer_modify(this.form);">수정</button>
                            </div>
                        </c:if>
                    </div>
                </article>
