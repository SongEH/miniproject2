...
//------페이지 목록 출력 -------------------------------------------------------------------------------------------------
		...
		for(int i=startPage; i<= endPage ;i++){
			...
			if(i == nowPage){ //현재 있는 페이지
				sb.append(String.format("<li><a class='active' href='list.do?page=#'>%d</a></li>", i));
...
//-----그룹페이지처리 다음 ----------------------------------------------------------------------------------------------
		if(isNextPage){
			sb.append(String.format("<li><a class='active' href='list.do?page=%d'>▶</a></li>", endPage+1));
		}
